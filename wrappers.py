...
class SelectObservationTransform(Transform):
    def __init__(self, in_keys=None, out_keys=None):
        ...
        super().__init__(in_keys=in_keys, out_keys=out_keys)
        self.choosen_index = [0, 1, 2, 3, 18, 19, 20, 21] 

    def _step(self, tensordict_in: TensorDictBase, tensordict_out: TensorDictBase = None) -> TensorDictBase:
        obs = tensordict_out.get(self.in_keys[0])
        tensordict_out.set(self.out_keys[0], obs[..., self.choosen_index])
        return tensordict_out

    def _reset(self, tensordict: TensorDictBase, tensordict_reset: TensorDictBase) -> TensorDictBase:
        obs = tensordict_reset.get(self.in_keys[0])
        tensordict_reset.set(self.out_keys[0], obs[..., self.choosen_index])
        return tensordict_reset

    def transform_observation_spec(self, observation_spec):
        old_spec = observation_spec[self.out_keys[0]]
        new_shape = observation_spec.shape + (len(self.choosen_index),)
...
class RescalingRewards(Transform):
    def __init__(self, in_keys=None, out_keys=None, rescaling_factor=1.0):
        if in_keys is None:
            in_keys = ["reward"]
        if out_keys is None:
            out_keys = in_keys  # overwrite in-place by default
    ...
    def _step(self, tensordict_in: TensorDictBase, tensordict_out: TensorDictBase) -> TensorDictBase:
        reward = tensordict_out.get(self.in_keys[0])
        tensordict_out.set(self.out_keys[0], reward * self.rescaling_factor)
        return tensordict_out
